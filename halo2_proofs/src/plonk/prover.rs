...
>(
    ...
    let lookups: Vec<Vec<lookup::prover::Permuted<Scheme::Curve, _>>> = instance_values
        .iter()
        ...
        .map(|(((instance_values, instance_cosets), advice_values), advice_cosets)| -> Result<Vec<_>, Error> {
            ...
            pk.vk
                ...
                .iter()
                .map(|lookup| {
                    lookup.commit_permuted::<Scheme,_,_,_,_,_  >(
                        pk,
                        params,
                        domain,
                        &value_evaluator,
    ...
    let permutations: Vec<permutation::prover::Committed<Scheme::Curve, _>> = instance
        .iter()
        .zip(advice.iter())
        .map(|(instance, advice)| {
            pk.vk.cs.permutation.commit::<Scheme, _, _, _, _>(
                params,
                pk,
                &pk.permutation,
    ...
    let lookups: Vec<Vec<lookup::prover::Committed<Scheme::Curve, _>>> = lookups
        .into_iter()
        .map(|lookups| -> Result<Vec<_>, _> {
            ...
            lookups
                .into_iter()
                .map(|lookup| {
                    lookup.commit_product::<Scheme, _, _, _>(
                        pk,
                        params,
                        theta,
                        beta,
    ...
    let vanishing =
        vanishing::Argument::commit::<Scheme, _, _, _>(params, domain, &mut rng, transcript)?;
    ...
    let vanishing = vanishing.construct::<Scheme, _, _, _, _, _>(
        params,
        domain,
        coset_evaluator,
        expressions,
