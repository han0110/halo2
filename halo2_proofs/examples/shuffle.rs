...
fn test_prover<C: CurveAffine, const W: usize, const H: usize>(
    ...
    let params = ParamsIPA::<C>::new(k, OsRng);
    let vk = keygen_vk::<IPACommitmentScheme<C>, _, ZK>(&params, &circuit).unwrap();
    let pk = keygen_pk::<IPACommitmentScheme<C>, _, ZK>(&params, vk, &circuit).unwrap();
    ...
    let accepted = {
        ...
        verify_proof::<IPACommitmentScheme<C>, _, _, VerifierIPA<C>, _, ZK>(
            &params,
            pk.get_vk(),
            strategy,
            &[&[]],
